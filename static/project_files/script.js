...
document.getElementById('table_head_row').addEventListener('click', event => {
    if(event.target){//If event target is not undefined
            //clear icons
            $('ion-icon').remove();
            ...
            if(['none', 'down'].includes(sortStatus[event.target.id])){
                ...
                //add icon
                event.target.innerHTML += '<ion-icon icon name="arrow-dropup-circle"></ion-icon>';
            }
                ...
                //add icon
                event.target.innerHTML += '<ion-icon icon name="arrow-dropdown-circle"></ion-icon>';
            }
